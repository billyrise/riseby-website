...
- **No.**: 12 / 163
- **slug**: ai-incident-response-escalation
- **title_ja**: エスカレーション閾値と重大基準の設計
- **template_id**: T13
- **unique_focus**: “エスカレーション閾値”に特化
...
### Points（書くべきポイント）
例外管理、教育、スピードと統制の両立（インセンティブ設計）

### Infographics（使うべきインフォグラフィック3種の「型」）
1) フロー（例外） 2) KPI/KRI 3) マトリクス（行動×統制）
