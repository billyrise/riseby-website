...
本稿の焦点は「“説明可能性のスコープ”」です。説明可能性のスコープについて、責任分界・証跡・監査提出の目次を具体化します。集中と分散のトレードオフを整理し、抜け穴がないようガードレールを置きます。 年1の監査では不足するため、継続適合性評価と自動化を運用に組み込みます。

## 本文

### 1. 評価設計

本稿の焦点は「“説明可能性のスコープ”」です。評価設計では、理念で終わらせず運用・証跡・監査可能性に落とすための要件を整理します。RACI、申請―審査―例外―更新、証跡、継続評価、インシデントの5要素のうち、このテーマに応じた項目を押さえます。集中と分散のトレードオフを整理し、抜け穴がないようガードレールを置きます。 年1の監査では不足するため、継続適合性評価と自動化を運用に組み込みます。

さらに、最小目次と落とし穴（目次欠落・日付不整合等）をテンプレで共有します。 攻撃パターンと対策を評価し、検知率・ブロック率をKRIで追います。

加えて、争点（権利・表示・証跡・方針）をチェックし、監査指摘を想定した対策を取ります。 調達審査のRACIと承認フローを文書化し、証跡の目次を監査と合意します。
...
監査で問われやすいのは「“説明可能性のスコープ”」に関して、責任分界と証跡の目次が揃っていないことです。明日から、継続評価の頻度と是正の流れをテンプレ化し、四半期の棚卸日をカレンダーに登録することを推奨します。
