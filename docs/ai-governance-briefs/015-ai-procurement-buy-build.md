...
- **対象読者**: Exec（Exec=経営層 / Pract=実務 / Audit=監査 / Legal=法務・コンプラ / Multi=複数）
- **目的**: Decide（Decide=意思決定 / Impl=実装 / Understand=理解 / Audit=監査対応 / Navigate=方向探索）
- **深さ**: Overview（Overview=1,500–2,000語 / Deep=2,500–3,500語 / Quick=800–1,200語）
- **スタイル**: Comparative（Narrative=物語調 / Analytical=分析調 / Prescriptive=処方箋調 / Comparative=比較調）
- **冒頭パターン（Op）**: 5 — ストーリー
  - 例: 「ある金融機関がAI導入を3回見送った理由—証跡設計の不備」
- **結論パターン（Co）**: 5 — 進化の視点
...
### Points（書くべきポイント）
open model/vendor eval、SBOM統合、供給網

### Infographics（使うべきインフォグラフィック3種の「型」）
1) サプライチェーン図 2) マトリクス（要件×ベンダー） 3) Evidence Chain
...
| 11 | ai-incident-reporting-authority | (未記録)() | (未記録)() | (未記録)() |
| 12 | ai-incident-response-escalation | (未記録)() | (未記録)() | (未記録)() |
| 13 | ai-lifecycle-overview | (未記録)() | (未記録)() | (未記録)() |
| 14 | ai-output-copyright | (未記録)() | (未記録)() | (未記録)() |

## 直前5記事で使用された形式（各形式1回までしか使えない）

直近5記事の図で使われた形式: **Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid**  
