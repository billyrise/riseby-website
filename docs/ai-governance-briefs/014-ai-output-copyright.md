...
- **No.**: 14 / 163
- **slug**: ai-output-copyright
- **title_ja**: 出力物の権利・責任と著作権
- **template_id**: T31
- **unique_focus**: “出力物の権利・責任”に特化
- **対象読者**: Legal（Exec=経営層 / Pract=実務 / Audit=監査 / Legal=法務・コンプラ / Multi=複数）
- **目的**: Understand（Decide=意思決定 / Impl=実装 / Understand=理解 / Audit=監査対応 / Navigate=方向探索）
...
- **冒頭パターン（Op）**: 4 — 対比
  - 例: 「理想：AIガバナンスで競争優位。現実：規程が形骸化し誰も読まない」
- **結論パターン（Co）**: 4 — リスク喚起
  - 例: 「放置した場合のワーストシナリオと、回避策」
...
### Points（書くべきポイント）
output copyright、training data licensing、表示義務

### Infographics（使うべきインフォグラフィック3種の「型」）
1) マトリクス（用途×リスク×対応） 2) レイヤー図 3) 差し戻しTop5（法務観点）
