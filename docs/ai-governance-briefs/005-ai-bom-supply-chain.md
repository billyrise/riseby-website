...
- **No.**: 5 / 163
- **slug**: ai-bom-supply-chain
- **title_ja**: AI-BOMと供給網の責任分界
- **template_id**: T25
- **unique_focus**: “供給網の責任分界”に特化
...
直近5記事の図で使われた形式: **Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, SVG, HTML, Table**  
...
## 図1（形式名）
（形式は SVG / HTML / Table / Canvas / Vega-Lite / Plotly / DOT / PlantUML 等から選択。Mermaidは禁止。該当するコードブロックのみ。例: ## 図1（SVG）の次に ```svg または ``` でSVGを書く。）
