...
- **No.**: 6 / 163
- **slug**: ai-cyber-threat-model
- **title_ja**: AIサイバー脅威モデルと優先順位付け
- **template_id**: T23
- **unique_focus**: “脅威分類の優先順位付け”に特化
...
- **冒頭パターン（Op）**: 6 — 統計インパクト
  - 例: 「AIインシデント報告、73%が『証拠不足で原因特定不能』」※数値は出典付きのみ可
- **結論パターン（Co）**: 1 — 行動への橋渡し
  - 例: 「明日から始められる3つのステップ」
...
### Points（書くべきポイント）
AI cyber threat model、攻撃面、優先統制

### Infographics（使うべきインフォグラフィック3種の「型」）
1) マトリクス（脅威×統制） 2) ライフサイクル輪 3) KPI/KRI
...
直近5記事の図で使われた形式: **Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, Mermaid, SVG, HTML, Table, SVG, HTML, Table**  
